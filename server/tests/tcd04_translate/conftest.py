"""
Fixtures for TCD-04 (Translation API) unit tests.

Infrastructure follows TCD-01/02/03 conventions:
  - coproof_test_db + NullPool + pg_terminate_backend + os.environ patch
  - TRUNCATE users CASCADE before each test for isolation

Additional fixtures
  - auth_user     : a plain User row + JWT, for tests that need authentication
  - user_with_key : an authenticated user who already has a saved UserApiKey
                    for "openai/gpt-4o"
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Minimal testing configuration (identical to TCD-01/02/03)
# ─────────────────────────────────────────────────────────────────────────────
...
def auth_user(app):
    """
    A basic authenticated user (no GitHub token needed for translate tests).
    Returns ``{id, token}``.
    """
